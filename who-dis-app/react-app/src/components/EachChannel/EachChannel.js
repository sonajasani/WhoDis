    ...
    <>
        <div className='channel-container' onClick={() => displayChannel()}>
            <div className='channel-each'>
                <NavLink id='channel-each-link'to={`/servers/${serverId}/${channelInfo?.id}`}>
                ...
                <div className='edit-channel-container'>
                    <h2 id='edit-channel-title'>Edit Channel Name</h2>

                    <ul id='edit-channel-errors'>
                    {
                        errors.length > 0 && errors.map((err, i) => (
                            <li id='edit-channel-li' key={i}>{err}</li>
                        ))
                    }
                    </ul>
                    ...
                    <form className='edit-channel-form' onSubmit={submitEdit}>
                        ...
                        <button type='submit' id='submit-edit-channel' disabled={!!errors.length}>Submit</button>
                        </div>
                    </form>
                </div>
                </Modal>
